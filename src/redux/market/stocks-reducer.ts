...
let initialState = {
  // возможно ли создать рынок акций...
  isAbleToGenerateStocks: false,
  // изменение цены . . .
  normalPriceChange: 3,
...
export const stocksReducer = (
  state = initialState,
  action: StocksActionType
): InitialStocksStateType => {
  switch (action.type) {
    ...
    case SET_STOCKS:
      ...
      state.companiesForStocks.forEach((company) => {
        ...
        if (Number(Math.random().toFixed(2)) < 0.35) {
          ...
          dividendsPercentage =
            Math.round(Math.random() * state.normalPriceChange) + 2;
        }
        ...
        let stock: stockType = {
          ...
          count: Number(
            (Math.random() * state.normalPriceChange * (risk + 1) + 1).toFixed(
              0
            )
          ),
          // риск акции
          ...
          dividendsAmount: Number(
            ((dividendsPercentage * price) / 100).toFixed(2)
          ),
          // макс цена акции
          maxPrice: Number(
            (
              price +
              Math.random() * state.normalPriceChange * (risk + 1)
            ).toFixed(1)
          ),
          // мин цена акции
          minPrice: Number(
            (price - Math.random() * state.normalPriceChange * risk).toFixed(1)
          ),
        };
    ...
    case SELL_STOCKS:
      ...
      stocksCopyToSell.forEach((s, i) => {
        if (s.title === action.stock.title)
          stocksCopyToSell[i] = {
            ...stocksCopyToSell[i],
            count: stocksCopyToSell[i].count + action.stock.count,
          };
      });
      // обновляем количество акций в фильтре...
      filteredStocksCopyToSell.forEach((s, i) => {
        if (s.title === action.stock.title)
          filteredStocksCopyToSell[i] = {
            ...filteredStocksCopyToSell[i],
            count: filteredStocksCopyToSell[i].count + action.stock.count,
          };
      });
    ...
    case INDEXING_STOCKS:
      ...
      state.stocks.forEach((stock, index) => {
        ...
        let indexPriceCount = Number(
          (stock.risk * Number((Math.random() + 0.1).toFixed(1))).toFixed(1)
        );
      ...
      switch (action.filter) {
        case "title":
          if (action.value === "") {
            ...
            filteredStocksCopy = [...state.stocks].filter((stock) =>
              stock.title.toLowerCase().includes(action.value.toLowerCase())
            );
          }
          break;
        ...
        case "price":
          filteredStocksCopy = state.stocks.sort(
            (prev, next) =>
              prev.price[prev.price.length - 1] -
              next.price[next.price.length - 1]
          );
          break;
      ...
      switch (action.filter) {
        case "title":
          if (action.value === "") {
            ...
            filteredBondsCopy = [...state.bonds].filter((bond) =>
              bond.title.toLowerCase().includes(action.value.toLowerCase())
            );
          }
          break;
        ...
        case "price":
          filteredBondsCopy = state.bonds.sort(
            (prev, next) =>
              prev.price[prev.price.length - 1] -
              next.price[next.price.length - 1]
          );
    ...
    case SET_BONDS:
      ...
      let count = Array.from(
        Array(getRandomNumber(10) + 10).fill({}),
        (item, index) => {
          return index;
        }
      );
        ...
        let title =
          getRandomNumber(10) > 5
            ? "ОФЗ " +
              ...
              " " +
              state.companiesForBonds[
                getRandomNumber(state.companiesForBonds.length)
              ]
            : state.companiesForBonds[
                getRandomNumber(state.companiesForBonds.length)
              ] +
              " выпуск " +
              String(getRandomNumber(20) + 1);
    ...
    case INDEXING_BONDS:
      ...
      state.bonds.forEach((bond, index) => {
        ...
        indexingBondsCopy[index] = {
          ...
          // новое количество акций . . .
          count:
            indexingBondsCopy[index].count + indexCount > 0
              ? indexingBondsCopy[index].count + indexCount
              : indexingBondsCopy[index].count,
          // новая цена акции . . .
          price: [...indexingBondsCopy[index].price, indexPrice],
          // новые выплаты с девидендов . . .
          dividendsAmount: Number(
            (
              (indexingBondsCopy[index].dividendsPercentage * indexPrice) /
              100
            ).toFixed(2)
          ),
        };
...
export const stocksActions = {
  ...
  updateMyStocks: (myStocks: myStockType[]) =>
    ({ type: UPDATE_MY_STOCKS, myStocks } as const),
  updateStocks: (title: string, count: number) =>
    ({ type: UPDATE_STOCKS, title, count } as const),
  sellStocks: (stock: myStockType, count: number, activeStock: number) =>
    ({ type: SELL_STOCKS, stock, count, activeStock } as const),
  ...
  filterStocks: (filter: filterType, value: string) =>
    ({ type: FILTER_STOCKS, filter, value } as const),
  reverseFilteredStocks: () => ({ type: REVERSE_FILTERED_STOCKS } as const),

  filterBonds: (filter: filterType, value: string) =>
    ({ type: FILTER_BONDS, filter, value } as const),
  reverseFilteredBonds: () => ({ type: REVERSE_FILTERED_BONDS } as const),
  buyInvestorJournal: () => ({ type: BUY_INVESTOR_JOURNAL } as const),
  setNewStocks: (newStocks: stockType[], newMyStocks: myStockType[]) =>
    ({ type: SET_NEW_STOCKS, newStocks, newMyStocks } as const),
  setInstructionCompleted: () => ({ type: SET_INSTRUCTION_COMPLETED } as const),
  setBrokers: () => ({ type: SET_BROKERS } as const),
  updateBrokerStocksCount: (brokers: brokerType[]) =>
    ({ type: UPDATE_BROKERS_STOCKS_COUNT, brokers } as const),
  indexStocksSummaryPrice: () =>
    ({ type: INDEX_STOCKS_SUMMARY_PRICE } as const),
  // создаем список облигаций..
  setBonds: () => ({ type: SET_BONDS } as const),
  // добавляем обязанность по марже...
  setMargin: (activeMargin: MarginType) =>
    ({ type: SET_MARGIN, activeMargin } as const),
...
export const addStocksToPortfolioThunk =
  (stock: stockType, count: number): ActionThunkType =>
  (dispatch, getState) => {
    ...
    if (myStocksCopy.some((s) => s.title === newStock.title)) {
      myStocksCopy = myStocksCopy.map((stock, index) => {
        if (stock.title === newStock.title) {
          let oldPrice = Number(
            (
              (stock.count * stock.oldPrice +
                newStock.count * newStock.oldPrice) /
          ...
          return {
            ...
            oldPrice: oldPrice,
            condition:
              stock.price >= oldPrice ? conditions.UP : conditions.DOWN,
          };
        }
        return stock;
      });
...
export const removeStocksFromPortfolioThunk =
  (stock: myStockType, count: number, index: number): ActionThunkType =>
  (dispatch, getState) => {
    //
...
export const updateBrokerStocksCountThunk =
  (broker: brokerType, count: number, stockTitle: string): ActionThunkType =>
  (dispatch, getState) => {
      ...
      if (b.name === broker.name) {
        b.stocks.forEach((s, i) => {
          if (s.title === stockTitle) {
            brokersCopy[index].stocks[i].count =
              brokersCopy[index].stocks[i].count - count;
          }
        });
      }
    });
...
export const addMarginToPortfolioThunk =
  (
    stock: stockType,
    broker: brokerType,
    count: number,
...
export const payMarginPenaltyThunk =
  (): ActionThunkType => (dispatch, getState) => {
    if (getState().stocksPage.margin.length > 0) {
      ...
      if (
        marginCopy.giveBackData.day === currentDay &&
        marginCopy.expiresIn > 0
      ) {
...
export const marginPayOutThunk =
  (count: number): ActionThunkType =>
  (dispatch, getState) => {
    let marginCopy = { ...getState().stocksPage.margin[0] };
    let myStocksCopy = [...getState().stocksPage.myStocks];
    let price = myStocksCopy.filter((s) => s.title === marginCopy.stockTitle)[0]
      .price;
    ...
    myStocksCopy.forEach((s, i) => {
      if (s.title === marginCopy.stockTitle) {
        ...
        if (myStocksCopy[i].count === 0) myStocksCopy.splice(i, 1);
      }
    });
    dispatch(stocksActions.updateMyStocks(myStocksCopy));
    // запись в историю...
    ...
    if (marginCopy.type === "long")
      dispatch(
        actions.updateWallet(price * count - marginCopy.stockPrice * count)
      );
...
type ActionThunkType = ThunkAction<
  ...
  AppStateType,
  unknown,
  StocksActionType | GameActionsType
>;

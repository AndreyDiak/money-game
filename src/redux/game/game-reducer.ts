...
let initialState = {
  ...
  // месяцы игры . . .
  months: [
    { name: "Январь", duration: 31 },
    { name: "Февраль", duration: 28 },
    { name: "Март", duration: 31 },
    { name: "Апрель", duration: 30 },
    { name: "Май", duration: 31 },
    { name: "Июнь", duration: 30 },
    { name: "Июль", duration: 31 },
    { name: "Август", duration: 31 },
    { name: "Сентябрь", duration: 30 },
    { name: "Октябрь", duration: 31 },
    { name: "Ноябрь", duration: 30 },
    { name: "Декабрь", duration: 31 },
  ],
};
...
export const actions = {
  ...
  updateWalletFromSpends: (wallet: number) =>
    ({ type: UPDATE_WALLET_FROM_SPENDS, wallet } as const),
  getNewsPayout: (payout: "one" | "regular", amount: number) =>
    ({ type: GET_NEWS_PAYOUT, payout, amount } as const),

  updateHistory: (history: any) => ({ type: UPDATE_HISTORY, history } as const),
  setVictoryBalance: (victory: number) =>
    ({ type: SET_VICTORY_BALANCE, victory } as const),
  // actions для бизнесса . . .
  buyBusiness: (price: number, income: number) =>
    ({ type: BUY_BUSINESS, price, income } as const),
  sellBusiness: (price: number, income: number) =>
    ({ type: SELL_BUSINESS, price, income } as const),
  updateBusinessIncome: (income: number) =>
    ({ type: UPDATE_BUSINESS_INCOME, income } as const),
...
export const updateMonthThunk = (): ActionThunkType => (dispatch, getState) => {
  ...
  if (
    (income >= 1000 && spendsLevel === 1) ||
    (income >= 4500 && spendsLevel === 2)
  ) {
  ...
  if (
    (income < 1000 && spendsLevel === 2) ||
    (income < 4500 && spendsLevel === 3)
  ) {
    dispatch(spendsActions.decreaseSpendsLevel());
    dispatch(spendsActions.setEventsPrice());
  }
};
export const balanceCheckThunk =
...
export const updateHistoryThunk =
  (
    target: any,
...
export const setPopupsShownThunk =
  (type: PopupsType, isShown: boolean): ActionThunkType =>
  (dispatch, getState) => {
...
export const setPopupsActiveThunk =
  (type: PopupsType, active: any): ActionThunkType =>
  (dispatch, getState) => {
    let popupsCopy = { ...getState().gamePage.popups };
    // @ts-ignore

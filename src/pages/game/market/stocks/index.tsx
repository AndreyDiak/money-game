          ...
          <div className="container">
            <div className="gameProfitMenu">
                      ...
                      <>
                        {index !== 0 && (
                          <div className="gameProfitMenu__item">
                            <Button
                              onClick={() => setMarketActiveFilter(index)}
                            >
                              {filter.name}
                            </Button>
                          </div>
              ...
              {screenWidth <= 768 && (
                <Select
                  defaultValue={1}
                  onChange={(value) => setMarketActiveFilter(value)}
                >
                  {filters.map((f, i) => {
            ...
            {screenWidth > 768 ? (
              <>
                {/* 2 columns */}
                <MyPortfolio />
                {marketActiveFilter === 1 && <Stocks />}
                {marketActiveFilter === 2 && <Bonds />}
                {marketActiveFilter === 3 && <Margin />}
              </>
            ) : (
              <>
                {/* 1 column */}
                {marketActiveFilter === 0 && <MyPortfolio />}
                {marketActiveFilter === 1 && <Stocks />}
                {marketActiveFilter === 2 && <Bonds />}
                {marketActiveFilter === 3 && <Margin />}
              </>
